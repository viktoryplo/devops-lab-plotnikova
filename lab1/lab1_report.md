University: [ITMO University](https://itmo.ru/ru/)  
Faculty: [FICT](https://fict.itmo.ru)  
Course: [Введение в веб технологии](https://itmo-ict-faculty.github.io/introduction-in-web-tech/)  
Year: 2025/2026  
Group: U4125  
Author: Плотникова Виктория Артемовна  
Lab: Lab0  
Date of create: 01.03.2026  
Date of finished:  

Отчет:
1) Скачала Docker на Windows, обновила весию Linux на компьютере, проверила установку и поработала с базовыми командами:  
![](test.png)  
2) Скачала образ Ububtu, запустила интерактивный контейнер и пакет 'curl':
![](ubuntu.png)
![](curl.png)
3) Запустила контейнер nginx и проверила работу в браузере, все работает корректно:
![](alpine.png)
![](nginx.png)
4) Посмотрела запущенные контейнеры, все контейнеры, остановила контейнер, запустила установленный, остановила, удалила образ:
![](allcont.png)
5) Далее по инструкции создала том, запустила контейнер с томом, подключила и создала файл через: echo "Hello from volume" > /data/test.txt:
![](uprconteyn.png)  
7) Удалила контейнер и создала новый с тем же томом + проверила, что файл сохранился:
![](check.png)
